      ...
      ?>
          ...
          <div class="bid-types tc">
			...
			<?php if( !empty( $bidDets ) ): ?>
								...
								<div class="col-xs-12 col-sm-10 col-md-10 col-lg-11 p0-xs">
									<div class="dash-bid-cont col-xs-12 col-sm-12 col-md-12 col-lg-6">
										...
										<?php if( !empty($bd['contracts']) and $bd['contracts']['workeragree'] == '0'): ?>
											<p class="pendingalert">Work Awarded. Awaiting Contractor Approval.</p>
									<?php endif; ?>
									</div>
					...
					<?php else: ?>
						<?php foreach( $bidDets as $bt => $bd ): 
							if (empty($bd['contracts']['contractkey']) ){
								continue;
							}
						?>
							<div class="dash-bids clearfix">
								...
								<div class="col-xs-12 col-md-12 col-lg-6 mt20-sm">
									<div class="col-sm-6 pl5 p0-xs">
										...
										<?php 
										if($bidtype == "completed"){
										?>
										<a target="_blank" href="<?php echo base_url('view-rating/'.$bd['jobkey'].'/'.$bd['contractorDets']['userkey'])?>" title="View Public Rating">Public Rating</a> | <a target="_blank" href="<?php echo base_url('view-rating/'.$bd['jobkey'].'/'.$bd['contractorDets']['userkey'].'?type=private')?>" title="View Private Rating">Private Rating</a>
										<?php 
										}
										?>
									</div>
									<?php if( $bidtype == 'working' ): ?>
										<div class="col-sm-6 p0 clearfix">
											<?php if( $bd['jobstatus'] == 'completed' ): ?>
												<a href="<?php echo base_url('rating/'.$bd['jobkey'].'/'.$bd['contractorDets']['userkey']); ?>" class="btn-green-accept minor-adjs" title="Accept Completion and Release Payment">Accept Completion and Release Payments</a>
											<?php else: ?>
												<?php if( !empty($bd['contracts']) and $bd['contracts']['workeragree'] == '0'): ?>
													<p class="pendingalert">Work Awarded. Awaiting Contractor Approval.</p>
									...
									<?php else: ?>
										<div class="col-sm-4 pr5 date-wraps pl0-xs"> <p>Completed on</p> <h5><?php echo date('m/d/Y',strtotime($bd['completiondate'])); ?></h5></div>
									<?php endif; ?>
								</div>
							</div>
						<?php endforeach; ?>
