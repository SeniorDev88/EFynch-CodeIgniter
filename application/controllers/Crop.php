          ...
          */
          ...
          if ( isset( $_POST[ "fileupload" ] ) ) {
              ...
              if( $ext != 'doc' || $ext != 'docx' || $ext == 'pdf' ){
                 list( $width, $height, $itype, $attr ) = getimagesize( $image[ 'tmp_name' ] );
              }
              $filetype = "image";
              if( $ext == 'doc' || $ext == 'docx' || $ext == 'pdf' ){
                $filetype = "doc";
			...
			if($filetype == "image" ){
              ...
              if ( $haserror != 1 ) {
                  ...
                  if($type == 'job'){
                    ...
                    if($filetype == "doc"){
                      $config[ 'upload_path' ]   = $tempCropStorage;
                    }
                  ...
                  if ( !$this->upload->do_upload( 'myfile' ) ) {
                      ...
                      $hwidth = (isset($uploadData[ "image_width" ])) ? $uploadData[ "image_width" ] : '';
                      $hheight = (isset($uploadData[ "image_height" ])) ? $uploadData[ "image_height" ] : '';
                      $this->User_model->create_temp_img( $imgKey, $ext,$hwidth , $hheight ,$_FILES[ 'myfile' ][ 'name' ]);
                      ...
                      $data['filetype'] = 'doc';
                      $data[ 'imagekey' ]  = $imgKey;
                      $data[ 'imagePath' ] = base_url() . $tempCropStorage . $fileName;
                      $data["view"]      = 3;
					  }else{
					  $data["view"]      = 4;  
					  }
			  ...
			  $haserror        = 1;
              $data[ 'error' ] = ($type == "job") ? "Please select an Image /Document to proceed" : "Please select an image to proceed";
		  }
              ...
              if ( !empty( $tempImageDetails )  and $_POST['skip'] == "1") {
                  $tempSource      =  base_url() .$tempStorage . $tempImageDetails[ "tempimgkey" ] . "." . $tempImageDetails[ "tempimgext" ];
				   ...
				   copy($tempSource,$cropDestination);
				          ...
				          $data['filetype'] = ( $tempImageDetails[ "tempimgext" ] == 'doc' || $tempImageDetails[ "tempimgext" ] == 'docx' || $tempImageDetails[ "tempimgext" ] == 'pdf' ) ? 'doc' : 'image';
